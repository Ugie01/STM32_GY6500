...
HAL_StatusTypeDef getAllDataOffset(GyData *dev)
{
    ...
    status = HAL_I2C_Mem_Read(dev->hi2c, dev->address, ACCEL_XOUT_H, 1, raw, 14, 100);
    if (status == HAL_OK)
    {
        dev->accel_x = ((float)((int16_t)((raw[0] << 8) | raw[1])) - dev->accel_offset_x) / ACCEL_SENSITIVITY;
        dev->accel_y = ((float)((int16_t)((raw[2] << 8) | raw[3])) - dev->accel_offset_y) / ACCEL_SENSITIVITY;
        dev->accel_z = ((float)((int16_t)((raw[4] << 8) | raw[5])) - dev->accel_offset_z) / ACCEL_SENSITIVITY;

        dev->temp = ((float)((int16_t)((raw[6] << 8) | raw[7])) / 340.0f) + 36.53f;

        dev->gyro_x = ((float)((int16_t)((raw[8] << 8) | raw[9])) - dev->gyro_offset_x) / GYRO_SENSITIVITY;
        dev->gyro_y = ((float)((int16_t)((raw[10] << 8) | raw[11])) - dev->gyro_offset_y) / GYRO_SENSITIVITY;
        dev->gyro_z = ((float)((int16_t)((raw[12] << 8) | raw[13])) - dev->gyro_offset_z) / GYRO_SENSITIVITY;
    }
...
HAL_StatusTypeDef setSensorState(GyData *dev, uint8_t set_data)
{
    ...
    status = HAL_I2C_Mem_Read(dev->hi2c, dev->address, PWR_MGMT_2, 1, &data, 1, TIME_OUT);
    if (status != HAL_OK)
        return status;
    ...
    return HAL_I2C_Mem_Write(dev->hi2c, dev->address, PWR_MGMT_2, 1, &data, 1, HAL_MAX_DELAY);
}
...
HAL_StatusTypeDef setTempState(GyData *dev, bool set_data)
{
    ...
    status = HAL_I2C_Mem_Read(dev->hi2c, dev->address, PWR_MGMT_1, 1, &data, 1, TIME_OUT);
    if (status != HAL_OK)
        return status;
    ...
    HAL_Delay(20); // 안정화될 때까지 잠시 대기
    return HAL_I2C_Mem_Write(dev->hi2c, dev->address, PWR_MGMT_1, 1, &data, 1, HAL_MAX_DELAY);
}
...
void calibrateGyro(GyData *dev, uint16_t samples)
{
    ...
    {
        HAL_I2C_Mem_Read(dev->hi2c, dev->address, GYRO_XOUT_H, 1, raw, 6, 100);
...
void calibrateAccel(GyData *dev, uint16_t samples)
{
    ...
    for (uint16_t i = 0; i < samples; i++)
    {
        HAL_I2C_Mem_Read(dev->hi2c, dev->address, ACCEL_XOUT_H, 1, raw, 6, 100);
        ax += (int16_t)((raw[0] << 8) | raw[1]);
        ay += (int16_t)((raw[2] << 8) | raw[3]);
        az += (int16_t)((raw[4] << 8) | raw[5]) - ONE_G_RAW; // 중력 보정
        HAL_Delay(2);
    }
